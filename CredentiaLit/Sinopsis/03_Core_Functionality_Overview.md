...
## 1. Verifiable Credential Issuance: Crafting Digital Truths

This is the process by which an entity (an educational institution, a community, an individual) asserts a claim about another entity (a student, a user, an asset) and cryptographically signs it.

*   **Key Functionalities:**
    *   **`issueCredential(issuerDid: string, subjectDid: string, credentialType: string, claims: object, conditions: LitCondition[])`:**
        *   Takes an `issuerDid` (Decentralized Identifier) and `subjectDid` (the entity the claim is about).
        *   Defines a `credentialType` (e.g., "CourseCompletion", "DiscordRoleMember", "DataAccessPermission").
    ...
    *   **Credential Lifecycle Management:** Functions to update or revoke credentials based on predefined conditions (e.g., `revokeCredential(credentialId: string, revocationConditions: LitCondition[])`).
*   **How it Works:** The SDK packages claims into a standardized VC format (e.g., W3C Verifiable Credentials), generates the necessary cryptographic signatures using the issuer's PKP orchestrated by Lit Nodes, and then typically stores a pointer (e.g., CID) or the VC itself on a decentralized storage layer (like MemoryChain) or directly to the subject's wallet.

## 2. Credential Verification: Proving Authenticity

Verification is the process by which a verifier (an application, another agent, a human) checks the validity and authenticity of a presented credential.
...
*   **Key Functionalities:**
    ...
    *   **`verifyIssuerIdentity(issuerDid: string) -> boolean`:** Confirms the authenticity of the issuer's Decentralized Identifier.
*   **How it Works:** The SDK interfaces with Lit Protocol's network to validate the signatures and conditions embedded within the VC. This process is entirely decentralized, meaning no single entity can falsely verify or deny a legitimate credential.

## 3. Decentralized Authorization: Conditional Access Orchestration

Authorization is about granting or denying access to resources based on verifiable credentials and conditions. This is where CredentiaLit unlocks incredibly powerful and flexible access control.
...
*   **Key Functionalities:**
    *   **`authorizeAccess(resource: string, userDid: string, requiredCredentials: CredentialRequirement[], accessConditions: LitCondition[]) -> boolean`:**
        *   Evaluates whether a `userDid` is authorized to access a `resource`.
        *   Checks if the user possesses the `requiredCredentials`.
        *   Evaluates arbitrary `accessConditions` using Lit Actions. Conditions can be based on:
            *   **On-chain data:** Does the user hold a specific NFT? Do they have a minimum token balance?
            *   **Off-chain data:** Does the user have a verified email? Is it within a specific time window? (This requires an Oracle or a securely attested claim).
            *   **Multiple criteria:** Combine conditions with AND/OR logic (e.g., "must hold NFT X AND have completed Course Y").
    *   **`encryptWithAccessControl(data: bytes, accessConditions: LitCondition[]) -> EncryptedData`:**
        *   Encrypts data such that only users who meet the specified `accessConditions` (evaluated by Lit Protocol) can decrypt it.
*   **How it Works:** The SDK converts application-level access rules into Lit Protocol's `LitConditions`. These conditions are then evaluated by the decentralized Lit network. If the conditions are met, the Lit Network releases a decryption key or signs a permission, allowing the authorized action to proceed. This completely decentralizes the access control layer, removing the need for a trusted third party.
